...
| 📖 主动叙事框架 | 参考 ProactiveAgent「节拍器」：开场/长时间静默时按手数间隔限频主动说一句**极短口语家常话**（老张/小雅各一组词库），且**真正交给数字人播报**（入记忆 + 入 avatar_command 播放链路），不打断 LLM 点评 |
...
│   │   │   ├── narrative_driver.py       # 主动叙事节拍器（简短家常话 + 手数限频）
...
- [ ] **第四阶段（打磨交付）**：交互打磨已完成 ✅（悔棋、棋谱库、人格切换、送吃保护与三结局兜底、AI 思考改为"让我想想…"）；**交互大改造已完成 ✅**（言语触发决策、数字人队列调度与情绪超时、引擎走法多样性、输入过滤、闲聊三档偏好、轻量人格、主动叙事框架、双凭证小雅）；**真人感收尾已完成 ✅**（绝杀判赢修复、说话频率下调、主动叙事台词真正播报，含 `scripts/cdp_e2e_check.js` CDP 前端 E2E 验证）；剩余：Prompt 调优、Docker-Compose 可复现、演示录屏、PDF 文档
