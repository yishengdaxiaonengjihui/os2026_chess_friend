...
> 镜像内已装 Node.js（引擎运行时）并拷贝 backend/frontend/vendor 三件套，`./data` 挂载持久化画像/棋谱；国内构建加速：`.env` 配 `PIP_INDEX_URL`（compose build 自动透传）。
