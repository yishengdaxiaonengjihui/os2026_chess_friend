...
| 🏆 绝杀判赢 | 引擎中局候选着过滤"送将"伪着法 + `is_checkmate` 字段修复 + 响应新增 `result`/`game_over` 字段，将死/困毙精确判胜（前端优先用 result 渲染，无需吃掉老将） |
...
| 📖 主动叙事框架 | 参考 ProactiveAgent「节拍器」：开场打招呼、长时间静默时按手数间隔限频主动说话——静默时**随机讲一条人格短回忆**（老张/小雅各有《棋王》《后翼弃兵》改编的回忆片段，回退家常语气词），**真正交给数字人播报**（入记忆 + 入 avatar_command 播放链路），不打断 LLM 点评 |
...
| 🗨️ 台词去机械化 | 内置口语库（老张/小雅各一组）：思考嘟囔、落子口头禅、吃子/将军/优劣势短句、用户长考 30s 主动搭话（整局≤2 次，简短语气词）；Prompt 硬性禁止"大爷您/我这下/机器腔"，人设梗概注入口癖；AI 落子前不再每手必嘟囔 |
| 🎤 语音输入闭环 | 对局室麦克风大按钮：Web Speech 中文识别 → 转写显示 → 后端 `/api/chat` 入记忆生成回复 → 数字人高优先级插播（barge-in）；噪音自动过滤、不支持时优雅降级 |
...
│   └── tests/                    # pytest 单测
├── scripts/                      # 演示/运维脚本（demo_engine.py）+ cdp_e2e_check.js（CDP 前端 E2E 验证）
...
- [ ] **第四阶段（打磨交付）**：交互打磨已完成 ✅（悔棋、棋谱库、人格切换、送吃保护与三结局兜底、AI 思考改为"让我想想…"）；**交互大改造已完成 ✅**（言语触发决策、数字人队列调度与情绪超时、引擎走法多样性、输入过滤、闲聊三档偏好、轻量人格、主动叙事框架、双凭证小雅）；**真人感收尾已完成 ✅**（绝杀判赢修复、说话频率下调、主动叙事台词真正播报 + 两位棋友人格故事落地，含 `scripts/cdp_e2e_check.js` CDP 前端 E2E 验证）；剩余：Prompt 调优、Docker-Compose 可复现、演示录屏、PDF 文档
