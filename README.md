...
| 🎲 走法多样性 | 开局库加权随机选着 + 中局在候选着中按胜率加权随机（非总走深搜最优），让棋友风格更像真人；**开局棋谱坐标已修正**（黑方顺手炮/屏风马/跳边马三条坐标错误：顺炮原先永远不合法、屏风马误写成往边角跳），棋谱只管**前 5 手**（`OPENING_BOOK_MAX_MOVE=5`，5 个半回合），之后纯引擎 |
...
| 📖 主动叙事框架 | 参考 ProactiveAgent「节拍器」：开场打招呼、长时间静默时按手数间隔限频主动说话——静默时**随机讲一条人格短回忆**（老张/小雅各有《棋王》《后翼弃兵》改编的回忆片段，回退家常语气词），**真正交给数字人播报**（入记忆 + 入 avatar_command 播放链路），不打断 LLM 点评；**故事已口语化改写**（只改语体不改内容：短句絮叨 + 各自口头禅，像"自己说自己的事"而非念稿）+ 按本回合事件关键词挑最贴合的片段（吃子/将军/马/车）+ 情绪/动作多样化（回忆偏沉思、得意内容配微笑）+ 会话内去重不反复讲同一段 |
