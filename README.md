...
- [ ] **第四阶段（打磨交付）**：交互打磨已完成 ✅（悔棋、棋谱库、人格切换、送吃保护与三结局兜底、AI 思考改为"让我想想…"）；**交互大改造已完成 ✅**（言语触发决策、数字人队列调度与情绪超时、引擎走法多样性、输入过滤、闲聊三档偏好、轻量人格、主动叙事框架、双凭证小雅）；**真人感收尾已完成 ✅**（绝杀判赢修复 + 全链路单测、说话频率下调、主动叙事台词真正播报 + 两位棋友人格故事落地、去"你/您"句式，含 `scripts/cdp_e2e_check.js` CDP 前端 E2E 验证——脚本同时校验后端响应含 result/game_over，可抓"旧后端未重启"回归）；剩余：Prompt 调优、Docker-Compose 可复现、演示录屏、PDF 文档
